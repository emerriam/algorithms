...
def judge_circle(moves)
  ...
  location = [0,0]
  moves.each do |move|
  	move == 'U' ? location[0] = location[0] - 1 : nil
  	move == 'D' ? location[0] = location[0] + 1 : nil
  	move == 'L' ? location[1] = location[1] - 1 : nil
  	move == 'R' ? location[1] = location[1] + 1 : nil
  end
  return true if location == [0,0]
  return false
end
